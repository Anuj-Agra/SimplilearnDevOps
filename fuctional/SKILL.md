...
| **frd-deep-dive** | Exhaustively analyses a single module and produces a detailed Functional Requirements Document with granular "the system shall..." statements, full data dictionaries, and every workflow diagrammed | `/mnt/skills/user/frd-deep-dive/SKILL.md` |
...
| **jira-generator** | Takes new requirements (optionally with existing spec) → produces JIRA-formatted tickets with 6-section anatomy | `/mnt/skills/user/jira-generator/SKILL.md` |
...
User wants a **detailed, low-level Functional Requirements Document** for a **single module** — granular "the system shall..." statements, full data dictionaries, every workflow diagrammed, testable by QA.

Signal phrases:
- "Deep dive into the [module] module"
- "Detailed FRD for [module]"
- "I need every requirement documented for this module"
- "Low-level functional requirements"
- "Exhaustive analysis of [module]"
- "Document every field, every rule, every workflow"
- "Generate the system shall statements for [module]"
- "Granular requirements for QA"
- User asks for detailed spec for ONE specific module (not the whole codebase)

**Action:** Read `/mnt/skills/user/frd-deep-dive/SKILL.md` and follow its instructions.

**When to choose frd-deep-dive vs spec-generator:**
- User wants the **whole system** at a high level → spec-generator
- User wants **one module** in exhaustive detail → frd-deep-dive
- User wants both → run spec-generator first for overview, then frd-deep-dive on priority modules
...
"I can help you with functional specifications in four ways:

1. **Generate** — Analyse your codebase and create a high-level functional spec across all modules
2. **Deep Dive** — Exhaustively analyse ONE module and produce a detailed FRD with every requirement, field, rule, and workflow
3. **Update Spec** — Take new requirements and show exactly what changes in your existing spec
4. **Create JIRA Tickets** — Turn requirements into formatted JIRA stories with acceptance criteria

Which would you like to do? (Or a combination — for example, generate the high-level spec first, then deep-dive a priority module.)"

---

## Shared Principles — Govern All Four Skills
...
- **frd-deep-dive:** Same rule with even higher stakes — every "the system shall..." statement must trace to a specific code observation. A fabricated FR creates a false requirement that wastes QA effort.
