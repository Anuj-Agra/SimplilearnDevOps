...
## Step 0 — Clarify Scope & Detect Codebase Root

### Auto-detect the codebase root

Determine the codebase root automatically based on what the user provides:

- **User uploads files** → root is `/mnt/user-data/uploads/` (or the top-level project folder within it)
- **User provides a path** like `/home/user/my-project` → that is the root
- **Running inside the repo** (e.g. Claude Code) → run `git rev-parse --show-toplevel 2>/dev/null || pwd` to detect the root

Store this as `CODEBASE_ROOT`. All output paths are **relative to this root** — use `./functional-specs/`, never an absolute path.
...
If detection fails or is ambiguous, ask: "I couldn't determine the codebase root automatically. Which directory is the top-level root of your project?"

### Confirm scope

1. **Target** — Whole mono-repo? Specific module? Sub-module?
2. **Known user roles** — Ask. If unknown, discover from code.
...
Work **one module at a time**. For each module, read the relevant code and generate 7 files (see `references/layer-templates.md` for exact templates):
...
| 06 | `06-flow-diagrams.md` | Mermaid diagrams: user workflows, state lifecycles, screen navigation |

For sub-modules, nest the same structure under `sub-modules/<name>/`.

### Generating Flow Diagrams (Layer 06)

Read `references/mermaid-conventions.md` for the full diagramming guide. For each module, generate diagrams **only for flows you can fully trace through the code**:

1. **User Workflow Diagrams** — One per major workflow. Source: controller call chains + frontend routes.
2. **Entity State Lifecycle Diagrams** — One per entity with states. Source: enum definitions + state-changing service methods.
3. **Screen Navigation Diagram** — How screens connect. Source: route definitions + routerLink targets.

If a flow is partially visible, diagram what you can see and add: `%% Partial flow — [section] not traceable in code`. Never invent steps.
...
| `system/06-system-flow-diagrams.md` | Mermaid diagrams: cross-module end-to-end journeys, auth flow, module interaction map |
...
Create the full tree under `./functional-specs/` relative to `CODEBASE_ROOT`. Also copy to `/mnt/user-data/outputs/functional-specs/` so the user can download from the conversation.

```bash
# Write to repo
mkdir -p "$CODEBASE_ROOT/functional-specs"
# ... generate all files ...

# Copy for download
cp -r "$CODEBASE_ROOT/functional-specs" /mnt/user-data/outputs/functional-specs
```

Read `references/output-structure.md` for the exact layout.
...
├── system/ (6 files)
├── modules/
│   └── <module-name>/ (7 files each + optional sub-modules/)
└── reference/ (4 files)
```
