...
| Flow Diagrams | [View →](./06-flow-diagrams.md) |
```
...
All Mermaid diagrams for the [Module Name] module. Each diagram traces directly
to code observations — no invented flows. See `references/mermaid-conventions.md`
for the diagramming guide.
...
flowchart TD
    A([Start]) --> B[Step from code]
    B --> C{Decision from code?}
    C -->|Yes| D[Outcome]
...
stateDiagram-v2
    [*] --> State1 : Created by [role]
    State1 --> State2 : [Role] performs [action]
    State2 --> [*]
` ` `
...
flowchart LR
    A(Screen 1) -->|User action| B(Screen 2)
    B -->|User action| C(Screen 3)
` ` `
```

(Remove spaces from triple backtick fences when generating actual output.)

### system/06-system-flow-diagrams.md
...
Mermaid diagrams showing cross-module flows, end-to-end user journeys,
and system-wide interaction patterns.
...
` ` `mermaid
%% SYS-MAP-001: Module Interactions
flowchart LR
    MOD_A(Module A) -->|relationship| MOD_B(Module B)
` ` `

## Authentication & Authorisation Flow

` ` `mermaid
%% SYS-AUTH-001: Login Flow
flowchart TD
